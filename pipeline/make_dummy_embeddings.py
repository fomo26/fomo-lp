...
Output format matches embed_all.py: one `<ptid>.npz` per subject, each holding
...
the `<ptid>.npz` filenames would collide).
"""
...
def make_dummy_embeddings(
    ...
    for dim in dims:
        ...
        # per-subject <ptid>.npz files don't overwrite each other.
        ...
        # One <ptid>.npz per subject, single 1-D array each (matches embed_all.py).
        for ptid, vec in zip(ptids, X):
            if not ptid:
                continue
            np.savez(dim_dir / f"{ptid}.npz", vec.ravel())
...
def main() -> None:
    ...
    parser.add_argument("--csv",  required=True, type=Path,
                        help="selected_subjects CSV (e.g. selected_subjects/selected_subjects.csv)")
    parser.add_argument("--out",  required=True, type=Path,
                        help="output directory for per-subject <ptid>.npz files")
    parser.add_argument("--dims", nargs="+", type=int, default=[128, 512, 2048],
                        help="embedding sizes to generate (default: 128 512 2048)")
    parser.add_argument("--label-col", default="label",
                        help="CSV column holding the class label (default: label)")
